...
exports.getMyStudyStatistics = async (req, res) => {
  try {
    ...
    let totalStudyDuration = 0; // 总学习时长 (分钟)
    ...
    reservations.forEach(reservation => {
      if (reservation.actualStartTime && reservation.actualEndTime) {
        ...
        totalStudyDuration += durationMinutes;
    ...
    // 计算平均学习时长
    const studyDays = Object.keys(dailyStudyDuration).length;
    const averageDailyDuration = studyDays > 0 ? Math.round(totalStudyDuration / studyDays) : 0;

    // 最长单日学习时长
    const maxDailyDuration = studyDays > 0 ? Math.max(...Object.values(dailyStudyDuration)) : 0;
